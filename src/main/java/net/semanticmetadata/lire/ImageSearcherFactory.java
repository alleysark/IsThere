...
public class ImageSearcherFactory {
    ...
    /**
     * Returns a new ChainedSearcher with given number of maximun hits and user selected descriptors
     * The weights of descriptor are not under 0, equal neither.
     *
     * @param maximumHits       defining how may hits are returned in max
     * @param descriptorWeights array of descriptor-weights class
     * @return the searcher instance or NULL if the weights are not appropriate.
     */
    public static ImageSearcher createChainedSearcher(int maximumHits, ArrayList<DescriptorWeight> descriptorWeights) {
        float weightSum = 0f;
        for (DescriptorWeight dw : descriptorWeights) {
            if (dw.weight <= 0f) return null;
            weightSum += dw.weight;
        }

        //Normalize weights
        for (DescriptorWeight dw : descriptorWeights) {
            dw.weight = dw.weight / weightSum;
        }
        return new ChainedSearcher(maximumHits, descriptorWeights);
    }
