...
public class SimpleDocumentBuilder extends AbstractDocumentBuilder {
    ...
    public Document createDocument(BufferedImage image, String identifier) {
        ...
        if (Math.max(image.getHeight(), image.getWidth()) > MAX_IMAGE_SIDE_LENGTH) {
            bimg = ImageUtils.scaleImage(image, MAX_IMAGE_SIDE_LENGTH);
        }
        ...
        if (sc != null)
            doc.add(new Field(FIELD_NAME_SCALABLECOLOR, sc, Field.Store.YES, Field.Index.NO));
        if (cl != null)
            doc.add(new Field(FIELD_NAME_COLORLAYOUT, cl, Field.Store.YES, Field.Index.NO));
        if (eh != null)
            doc.add(new Field(FIELD_NAME_EDGEHISTOGRAM, eh, Field.Store.YES, Field.Index.NO));
        if (identifier != null)
